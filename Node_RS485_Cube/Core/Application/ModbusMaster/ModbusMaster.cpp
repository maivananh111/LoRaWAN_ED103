...
@ingroup setup
*/
void ModbusMaster::begin(uint8_t slave, UART_HandleTypeDef *serial)
{
  _u8MBSlave = slave;
  _serial = serial;
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction){
  ...
  HAL_UART_Transmit(_serial, u8ModbusADU, u8ModbusADUSize, 1000);
  ...
  while (u8BytesLeft && !u8MBStatus){
    if (HAL_UART_Receive(_serial, &u8ModbusADU[u8ModbusADUSize], 1, ku16MBResponseTimeout) == HAL_OK){
      u8ModbusADUSize++;
      u8BytesLeft--;
    }
    else{
