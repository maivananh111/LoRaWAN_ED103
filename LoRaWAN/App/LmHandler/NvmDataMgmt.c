...
static uint16_t NvmNotifyFlags = LORAMAC_NVM_NOTIFY_FLAG_NONE;
...
uint16_t NvmDataMgmtRestore(void) {
	...
	if (nvm.SecureElement.Crc32 != 0xFFFFFFFFUL) {
		memcpy(mibReq.Param.Contexts, &nvm, sizeof(LoRaMacNvmData_t));
		return ( ( sizeof( LoRaMacNvmData_t ) + 7 ) & ~0x07 );
	}
#endif
	return 0;
}
