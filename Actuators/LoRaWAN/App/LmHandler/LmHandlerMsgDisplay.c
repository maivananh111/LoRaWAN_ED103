...
void DisplayMacMlmeRequestUpdate(LoRaMacStatus_t status, MlmeReq_t *mlmeReq, TimerTime_t nextTxIn) {
	switch (mlmeReq->Type) {
		...
		case MLME_TXCW:
			...
			break;
		case MLME_TXCW_1:
			LOGW(TAG, "MAC REQUEST TXCW_1: %s", MacStatusStrings[status]);
			break;
		default:
...
void DisplayBeaconUpdate(LoRaMAcHandlerBeaconParams_t *params) {
	switch (params->State) {
		...
		case LORAMAC_HANDLER_BEACON_ACQUIRING: {
			LOGI(TAG, "BEACON ACQUIRING");
			break;
		}
		case LORAMAC_HANDLER_BEACON_LOST: {
			LOGI(TAG, "BEACON LOST");
			break;
		}
		case LORAMAC_HANDLER_BEACON_RX: {
			LOGI(TAG, "BEACON %lu.", params->Info.Time.Seconds);
			LOGI(TAG, "GW DESC: %d", params->Info.GwSpecific.InfoDesc);
			LOGI(TAG, "GW INFO: %s", ToHexBuffer(params->Info.GwSpecific.Info, 6));
			LOGI(TAG, "FREQ   : %lu", params->Info.Frequency);
			LOGI(TAG, "DR     : DR_%d", params->Info.Datarate);
			LOGI(TAG, "RSSI   : %d", params->Info.Rssi);
			LOGI(TAG, "SNR    : %d", params->Info.Snr);
			break;
		}
		case LORAMAC_HANDLER_BEACON_NRX: {
			LOGI(TAG, "BEACON NOT RECEIVED.");
			break;
		}
	}
}
